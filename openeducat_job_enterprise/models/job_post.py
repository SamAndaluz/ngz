...
class OpPlacementOffer(models.Model):
    ...
    salary_from = fields.Integer("Salary From",
                                 track_visibility='always', required=True)
    salary_upto = fields.Integer("Salary Upto",
                                 track_visibility='always', required=True)
    payable_at = fields.Selection(
        [('weekly', 'Weekly'),
         ('monthly', 'Monthly'),
         ('yearly', 'Yearly')], track_visibility='always', string='Payable At')
    ...
    def create(self, vals):
        ...
        return super(OpPlacementOffer, self).create(vals)
